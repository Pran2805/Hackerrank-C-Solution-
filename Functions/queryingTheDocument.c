...
// we can't edit the above line , so we'll just ignore it

// char**** is 4 dimensional array n is paragraph m is sentence and n is word
char* kth_word_in_mth_sentence_of_nth_paragraph(char**** document, int k, int m, int n) {
    // simply returning the the value of the kth word in the mth sentence of the nth paragraph
...
char** split_string(char* text, char delim) {
    // we are dividing the content of the text by the delimiter and returning the result
    // assert run at runtime
    assert(text != NULL);
...
// we can't edit the code below, so we will create a new function to solve the problem
